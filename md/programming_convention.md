...
* 可読性が高い。
    ...
    * ファイルや識別子の名前に規則性があり、適切に命名されている(「[命名規則](---)」参照)。 
...
* 上記のようなunionはランタイム依存性が強いため、それへの依存を最小にする。
  従って、unionの定義を外部[パッケージ](---)に公開(「[ソースコードファイルとディレクトリ](---)」参照)しない。
...
* インスタンス、インスタンスへのポインタ、インスタンスへのリファレンス等に対して、
    * [constexpr変数](---)をパラメータにした場合、コンパイル時に定数として評価できる関数は、
...
- ソースコードファイル全体は、適切に[パッケージ](---)に分割する。
- ソースコードファイルの最小構成は[ファイルペア](---)を形成しなければならない。
...
* [ファイルペア](---)の実装ファイルは、対応するヘッダファイルをインクルードしなければならない。
...
* 不要/不適切な依存関係(「[インターフェース分離の原則(ISP)](---)」や
  「[依存関係逆転の原則(DIP)](---)」への違反)を作らない。
    ...
    * パッケージ間の依存関係は、[非循環依存の原則(ADP)](---)を守る。

![パッケージの構造(NG)](plant_uml/package_ng.png)

* ヘッダファイル間の依存関係を最小にとどめるために、
  前方宣言や[前方宣言ヘッダ(`_fwd.h`)](---)イディオムやを適切に使用する。
...
* 標準ライブラリのクラス(class, struct, enum, enum class)の前方宣言をしない
  ...
  このような前方宣言を行うと[ODR](---)違反による[未定義動作](---)につながる可能性がある)。
...
* ヘッダファイルが外部からインポートする型(class、struct、enum)
  ...
  前方宣言や[前方宣言ヘッダ(`_fwd.h`)](---)を使い依存関係を小さくする。 
...
## スコープ
### スコープの定義と原則
この章で扱うスコープを下記のように定義する(「[ソースコードファイルとディレクトリ](---)」参照)。
...
その識別子は[パッケージ](---)外部非公開なヘッダファイルに宣言、定義されているため、
...
* このドキュメント執筆時のコンパイラ/ビルダのC++20での[言語拡張機能|モジュール](---)のサポート状況が
  万全ではないため、
...
* オブジェクトのアライメントが必要な場合、
    * alignas、alignofを使用する(「[プレースメントnew](---)」参照)。
