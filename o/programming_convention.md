...
* 可読性が高い。
    ...
    * ファイルや識別子の名前に規則性があり、適切に命名されている
      (「[Name and Conquer](appendix.md#SS_13_1)」、「[命名規則](naming_practice.md#SS_4)」参照)。 
...
&emsp;&emsp;&emsp;&emsp; [ダイナミックメモリアロケーションの制約が強いシステム](programming_convention.md#SS_2_5_6_3)  
...
* コンストラクタ以外のメンバ関数を定義しない。
    * [ディープコピー](cpp_idioms.md#SS_11_9_2)(「[コンストラクタ](programming_convention.md#SS_2_2_4_2)」参照)が必要な型は、structでなくclassで表す。
...
* 列型オブジェクトは以下で述べるような問題や、
  [AAAスタイル](cpp_idioms.md#SS_11_8_1)で生成できない等の様々な問題を起こしやすいため、
...
* constは[west-const](cpp_idioms.md#SS_11_8_3)に従って記述する。
...
* [AAAスタイル](cpp_idioms.md#SS_11_8_1)に従い適切にautoを使用する。
...
* 関数内のオブジェクトは、出来る限り[AAAスタイル](cpp_idioms.md#SS_11_8_1)を用いて宣言し、同時に初期化する。
* [算術型](core_lang_spec.md#SS_9_1_3)の宣言に[AAAスタイル](cpp_idioms.md#SS_11_8_1)が使えない場合、
...
* リファレンスやポインタの宣言に[AAAスタイル](cpp_idioms.md#SS_11_8_1)が使えない場合、
...
* 構造体やクラス型オブジェクトの宣言に[AAAスタイル](cpp_idioms.md#SS_11_8_1)が使えない場合、
    * 「代入演算子を伴わない[一様初期化](core_lang_spec.md#SS_9_6_6)」を使用する。
...
* decltypeによるオブジェクトの宣言は、[AAAスタイル](cpp_idioms.md#SS_11_8_1)と同様に行う。
...
* 単なるデータホルダー(アプリケーションの設定データを保持するようなクラス等)や、
  ...
  [凝集性](cpp_idioms.md#SS_11_11_10)が高くなるように設計する。
...
* 非静的メンバの[ハンドル](cpp_idioms.md#SS_11_11_7)を返すメンバ関数を持つオブジェクトが
  [rvalue](core_lang_spec.md#SS_9_7_1_2)である場合、
  そのオブジェクトからその関数を呼び出した戻り値(メンバへのハンドル)を変数で保持しない
  (そのハンドルは[danglingリファレンス](cpp_idioms.md#SS_11_10_2)/[danglingポインタ](cpp_idioms.md#SS_11_10_3)になっている)。
...
* クラスが解放責務を持つポインタ型メンバ変数を持つならば、copyコンストラクタ、
  copy代入演算子に対して以下のいずれかを行い、[シャローコピー](cpp_idioms.md#SS_11_9_1)が行われないようにする
  (このルールはファイルディスクリプタ等のリソース管理をするクラス全般に当てはまる)。
    * [ディープコピー](cpp_idioms.md#SS_11_9_2)をさせる。
...
* [オーバーライドとオーバーロードの違い](cpp_idioms.md#SS_11_10_1)に注意する。
...
* オブジェクトの[スライシング](cpp_idioms.md#SS_11_9_3)には以下のいずれかで対処する。
    * [Clone(仮想コンストラクタ)](design_pattern.md#SS_7_1_3)を使用する。
    * copy代入演算子を= deleteする。

* [スライシング](cpp_idioms.md#SS_11_9_3)と類似の問題が起こるため、
...
* オブジェクトaの所有権を持たないオブジェクトは、
  オブジェクトaの[ハンドル](cpp_idioms.md#SS_11_11_7)をメンバ変数で保持することを出来る限り避ける
...
* クラスAのオブジェクトaが、オブジェクトbにダイナミックに生成されたとすると、  
    ...
    * オブジェクトbが`std::shared_ptr<A>`でオブジェクトaを、
      ...
      [std::weak_ptr](stdlib_and_concepts.md#SS_10_5_4)を適切に使用する。
...
* [danglingリファレンス](cpp_idioms.md#SS_11_10_2)、 [danglingポインタ](cpp_idioms.md#SS_11_10_3)に気を付ける。
...
* 実引数として使用される配列がポインタ型へ暗黙に変換されることを前提に、
  ...
  (「[スライシング](cpp_idioms.md#SS_11_9_3)」で述べたように、
...
* 関数、メンバ関数はなるべく[スレッドセーフ](cpp_idioms.md#SS_11_11_2)に実装する。
...
* try-catchが不可避である場合、以下の理由によりconstリファレンスで受け取る。
    * 実態で受け取るとオブジェクトの[スライシング](cpp_idioms.md#SS_11_9_3)が起こる場合がある。
...
* &&や||の論理演算子の右オペランドで[副作用](cpp_idioms.md#SS_11_11_13)のある処理をしない。
...
* `std::shared_ptr<>`でダイナミックに生成したオブジェクトを管理する場合、
  [オブジェクトの循環所有](cpp_idioms.md#SS_11_4_3)が発生しないように気を付ける(適切に[std::weak_ptr](stdlib_and_concepts.md#SS_10_5_4)を使う)。
...
* [new/deleteのオーバーロード](appendix.md#SS_13_2)する場合、newはnullptrを返さないようにする。
...
#### ダイナミックメモリアロケーションの制約が強いシステム <a id="SS_2_5_6_3"></a>
...
* MISRA/AUTOSAR C++等のヒープの使用制限が強い制約(リアルタイム性の遵守)を守る必要がある
  (「[malloc/freeの問題点](appendix.md#SS_13_3)」参照)。
...
* デフォルトのグローバルnewを使用しない。
    * リアルタイム性に制約のあるシステムでは、
      「[グローバルnew/deleteのオーバーロード](appendix.md#SS_13_4)」で述べたようなnewを実装する。
    * メモリ制限が強いシステムでは、ダイナミックなオブジェクト生成を避け、
      やむを得ない場合、「[クラスnew/deleteのオーバーロード](appendix.md#SS_13_5)」で述べたようなクラス毎のnewを実装する。
* malloc/newを用いてエクセプション送出を行っているツールチェーンを使用している場合、
 「[エクセプション処理機構の変更](appendix.md#SS_13_6)」で述べたような方法でリアルタイム制を確保するか、
  エクセプションを使用しない。
...
* 上記例を除き、sizeof演算子のオペランドは一見[副作用](cpp_idioms.md#SS_11_11_13)を持っているような式を含んではならない。
...
* パッケージに関して、以下の構造を持つようにソースコードを構成する
  (「[アーキテクチャとファイル構造](appendix.md#SS_13_7)」参照)。
    * ソフトウェアはパッケージに分割される。
...
* 識別子のインポートのための[using宣言](core_lang_spec.md#SS_9_12_14)は下記のような場合のみに使用する
  (「[継承コンストラクタ](core_lang_spec.md#SS_9_6_1_2)」、「[オーバーライドとオーバーロードの違い](cpp_idioms.md#SS_11_10_1)」参照)。
...
* [ディープコピー](cpp_idioms.md#SS_11_9_2)の実装を持つクラスへのcopy代入の多くがrvalueから行われるのであれば、
...
* [g++](cpp_idioms.md#SS_11_12_1)/[clang++](cpp_idioms.md#SS_11_12_2)等の優れたコンパイラを適切なオプションで使用することで、
  非推奨の機能、関数、クラスの使用を防ぐ。
...
```
    ...
    localtime(), ttyname(), 
    ctermid(), tmpnam() (引数がNULLのとき、非[リエントラント](cpp_idioms.md#SS_11_11_3)になる)
```
...
* signalの扱いは極めて難しく、安定動作をさせるのは困難である。
  「シグナルの[リエントラント](cpp_idioms.md#SS_11_11_3)問題を解決でき、使用できる関数に制限がない」という利点があるため、
...
* assert()はコンパイルオプションにより無効化されることがあるため、
  assert()の引数に[副作用](cpp_idioms.md#SS_11_11_13)のある式を入れない。
