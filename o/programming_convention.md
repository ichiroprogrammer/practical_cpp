...
* コンストラクタ以外のメンバ関数を定義しない。
    * [ディープコピー](cpp_idioms.md#SS_12_12_2)(「[コンストラクタ](programming_convention.md#SS_3_2_4_2)」参照)が必要な型は、structでなくclassで表す。
...
* 上記のようなunionはランタイム依存性が強いため、それへの依存を最小にする。
  従って、unionの定義を外部[パッケージ](cpp_idioms.md#SS_12_9_2)に公開(「[ソースコードファイルとディレクトリ](programming_convention.md#SS_3_7)」参照)しない。
...
* 列型オブジェクトは以下で述べるような問題や、
  [AAAスタイル](cpp_idioms.md#SS_12_11_1)で生成できない等の様々な問題を起こしやすいため、
...
* constは[west-const](cpp_idioms.md#SS_12_11_3)に従って記述する。
...
* [AAAスタイル](cpp_idioms.md#SS_12_11_1)に従い適切にautoを使用する。
...
* 関数内のオブジェクトは、出来る限り[AAAスタイル](cpp_idioms.md#SS_12_11_1)を用いて宣言し、同時に初期化する。
* [算術型](core_lang_spec.md#SS_10_1_3)の宣言に[AAAスタイル](cpp_idioms.md#SS_12_11_1)が使えない場合、
...
* リファレンスやポインタの宣言に[AAAスタイル](cpp_idioms.md#SS_12_11_1)が使えない場合、
...
* 構造体やクラス型オブジェクトの宣言に[AAAスタイル](cpp_idioms.md#SS_12_11_1)が使えない場合、
    * 「代入演算子を伴わない[一様初期化](core_lang_spec.md#SS_10_6_6)」を使用する。
...
* decltypeによるオブジェクトの宣言は、[AAAスタイル](cpp_idioms.md#SS_12_11_1)と同様に行う。
...
* 単なるデータホルダー(アプリケーションの設定データを保持するようなクラス等)や、
  ...
  [凝集性](cpp_idioms.md#SS_12_14_14)が高くなるように設計する。
...
* 全てのメンバ変数はprivateにする。
    * メンバ変数にアクセスしたい場合は、Accessorメンバ関数を経由させる
      (「[Accessor](cpp_idioms.md#SS_12_1_6)」参照)。その場合でもsetterは控えめに使用する。
...
* [NVI(non virtual interface)](cpp_idioms.md#SS_12_1_8)に従う。従って、
...
* 非静的メンバの[ハンドル](cpp_idioms.md#SS_12_14_8)を返すメンバ関数を持つオブジェクトが
  [rvalue](core_lang_spec.md#SS_10_7_1_2)である場合、
  そのオブジェクトからその関数を呼び出した戻り値(メンバへのハンドル)を変数で保持しない
  (そのハンドルは[danglingリファレンス](cpp_idioms.md#SS_12_13_2)/[danglingポインタ](cpp_idioms.md#SS_12_13_3)になっている)。
...
* リソース管理等の都合からコンパイラが生成するデストラクタでは機能が不十分な場合、
  ...
  これらを使用しない(「[Copy-And-Swap](cpp_idioms.md#SS_12_1_4)」参照)。
...
* クラスが解放責務を持つポインタ型メンバ変数を持つならば、copyコンストラクタ、
  copy代入演算子に対して以下のいずれかを行い、[シャローコピー](cpp_idioms.md#SS_12_12_1)が行われないようにする
  (このルールはファイルディスクリプタ等のリソース管理をするクラス全般に当てはまる)。
    * [ディープコピー](cpp_idioms.md#SS_12_12_2)をさせる。
    * = deleteする(「[特殊メンバ関数](core_lang_spec.md#SS_10_6_1)」参照)。

  またこの場合、moveコンストラクタ、move代入演算子の定義を検討する(「[Copy-And-Swap](cpp_idioms.md#SS_12_1_4)」参照)。
...
* [オーバーライドとオーバーロードの違い](cpp_idioms.md#SS_12_13_1)に注意する。
...
* privateやprotectedなオーバーライド関数にはデフォルト引数を持たさない
  (「[実引数/仮引数](programming_convention.md#SS_3_3_4)」参照)。
  さらに[NVI(non virtual interface)](cpp_idioms.md#SS_12_1_8)にも従うことにより、
...
* 実装の継承よりも、包含、委譲を優先的に使用する。やむを得ず実装の継承を行う場合は、
  ...
  ([CRTP(curiously recurring template pattern)](cpp_idioms.md#SS_12_1_5)等は例外的に認められる)。
...
* C#やJavaのinterfaceが必要ならば(インタフェースと実装の完全分離をしたい場合等)、
  ...
  (もしくはそのクラスに[NVI(non virtual interface)](cpp_idioms.md#SS_12_1_8)を適用したクラス)
  を定義する。
...
* オブジェクトの[スライシング](cpp_idioms.md#SS_12_12_3)には以下のいずれかで対処する。
    * [Clone(仮想コンストラクタ)](design_pattern.md#SS_8_1_3)を使用する。
    * copy代入演算子を= deleteする。

* [スライシング](cpp_idioms.md#SS_12_12_3)と類似の問題が起こるため、
...
* オブジェクトaの所有権を持たないオブジェクトは、
  オブジェクトaの[ハンドル](cpp_idioms.md#SS_12_14_8)をメンバ変数で保持することを出来る限り避ける
  ([Observer](design_pattern.md#SS_8_2_4)パターン等、このルール順守が困難な場合は多い)。
* クラスAのオブジェクトaが、オブジェクトbにダイナミックに生成されたとすると、  
    * オブジェクトaのポインタは`std::unique_ptr<A>`(「[RAII(scoped guard)](cpp_idioms.md#SS_12_1_3)」参照)で保持する。
...
* [danglingリファレンス](cpp_idioms.md#SS_12_13_2)、 [danglingポインタ](cpp_idioms.md#SS_12_13_3)に気を付ける。
...
* 演算子をオーバーロードする場合、それが自然に使えるようにする。
    ...
    * copy(またはmove)代入演算子を定義する場合、copy(またはmove)コンストラクタも定義する
      (その際、コードクローンを作りがちなので注意する(「[Copy-And-Swap](cpp_idioms.md#SS_12_1_4)」参照))。
...
* 実引数として使用される配列がポインタ型へ暗黙に変換されることを前提に、
  ...
  (「[スライシング](cpp_idioms.md#SS_12_12_3)」で述べたように、
...
* 関数、メンバ関数はなるべく[スレッドセーフ](cpp_idioms.md#SS_12_14_3)に実装する。
...
* try-catchが不可避である場合、以下の理由によりconstリファレンスで受け取る。
    * 実態で受け取るとオブジェクトの[スライシング](cpp_idioms.md#SS_12_12_3)が起こる場合がある。
    * 受け取ったエクセプションオブジェクトを書き換えるべきではない。

* エクセプションによるリソースリークを避けるため[RAII(scoped guard)](cpp_idioms.md#SS_12_1_3)でリソースを管理する。
...
* &&や||の論理演算子の右オペランドで[副作用](cpp_idioms.md#SS_12_14_16)のある処理をしない。
...
* 上記例を除き、sizeof演算子のオペランドは一見[副作用](cpp_idioms.md#SS_12_14_16)を持っているような式を含んではならない。
...
- ソースコードファイル全体は、適切に[パッケージ](cpp_idioms.md#SS_12_9_2)に分割する。
- ソースコードファイルの最小構成は[ファイルペア](cpp_idioms.md#SS_12_9_1)を形成しなければならない。
...
* [ファイルペア](cpp_idioms.md#SS_12_9_1)の実装ファイルは、対応するヘッダファイルをインクルードしなければならない。
...
* 不要/不適切な依存関係(「[インターフェース分離の原則(ISP)](solid.md#SS_7_4)」や
  「[依存関係逆転の原則(DIP)](solid.md#SS_7_5)」への違反)を作らない。
    ...
    * パッケージ間の依存関係は、[非循環依存の原則(ADP)](cpp_idioms.md#SS_12_8_4)を守る。
...
* ヘッダファイル間の依存関係を最小にとどめるために、
  前方宣言や[前方宣言ヘッダ(`_fwd.h`)](cpp_idioms.md#SS_12_1_2)イディオムやを適切に使用する。
...
* ヘッダファイルが外部からインポートする型(class、struct、enum)
  ...
  前方宣言や[前方宣言ヘッダ(`_fwd.h`)](cpp_idioms.md#SS_12_1_2)を使い依存関係を小さくする。 
...
1. グローバル
2. [パッケージ](cpp_idioms.md#SS_12_9_2)外部公開名前空間
3. [パッケージ](cpp_idioms.md#SS_12_9_2)外部非公開名前空間
...
その識別子は[パッケージ](cpp_idioms.md#SS_12_9_2)外部非公開なヘッダファイルに宣言、定義されているため、
...
* グローバル名前空間には下記以外の識別子を定義、宣言しない。
    ...
    * グローバルnewのオーバーロード
    * Cとシェアする識別子
* [パッケージ](cpp_idioms.md#SS_12_9_2)毎に名前空間を定義する
...
* 識別子のインポートのための[using宣言](core_lang_spec.md#SS_10_12_14)は下記のような場合のみに使用する
  (「[継承コンストラクタ](core_lang_spec.md#SS_10_6_1_2)」、「[オーバーライドとオーバーロードの違い](cpp_idioms.md#SS_12_13_1)」参照)。
...
* [ディープコピー](cpp_idioms.md#SS_12_12_2)の実装を持つクラスへのcopy代入の多くがrvalueから行われるのであれば、
...
* [g++](cpp_idioms.md#SS_12_15_1)/[clang++](cpp_idioms.md#SS_12_15_2)等の優れたコンパイラを適切なオプションで使用することで、
  非推奨の機能、関数、クラスの使用を防ぐ。
...
* std::arrayを除くコンテナクラスは、
  それ自体でメモリリソースの[RAII(scoped guard)](cpp_idioms.md#SS_12_1_3)を実現しているため、newしない。
...
```
    ...
    localtime(), ttyname(), 
    ctermid(), tmpnam() (引数がNULLのとき、非[リエントラント](cpp_idioms.md#SS_12_14_4)になる)
```
...
* signalの扱いは極めて難しく、安定動作をさせるのは困難である。
  「シグナルの[リエントラント](cpp_idioms.md#SS_12_14_4)問題を解決でき、使用できる関数に制限がない」という利点があるため、
...
* assert()はコンパイルオプションにより無効化されることがあるため、
  assert()の引数に[副作用](cpp_idioms.md#SS_12_14_16)のある式を入れない。
