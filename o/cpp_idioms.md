...
&emsp;&emsp;&emsp; [プライオリティインバージョン](cpp_idioms.md#SS_11_12_2)  
&emsp;&emsp;&emsp; [スレッドセーフ](cpp_idioms.md#SS_11_12_3)  
&emsp;&emsp;&emsp; [リエントラント](cpp_idioms.md#SS_11_12_4)  
&emsp;&emsp;&emsp; [クリティカルセクション](cpp_idioms.md#SS_11_12_5)  
&emsp;&emsp;&emsp; [スピンロック](cpp_idioms.md#SS_11_12_6)  
&emsp;&emsp;&emsp; [ミックスイン](cpp_idioms.md#SS_11_12_7)  
&emsp;&emsp;&emsp; [ハンドル](cpp_idioms.md#SS_11_12_8)  
...
* copy代入演算子には、[エクセプション安全性の保証](core_lang_spec.md#SS_9_13_7)がない。
...
これにより[エクセプション安全性の保証](core_lang_spec.md#SS_9_13_7)を持つ4関数をコードクローンすることなく実装できる。
...
以下にこのパターンを使用した[ミックスイン](cpp_idioms.md#SS_11_12_7)の例を示す。
...
なお、このパターンは、[std::enable_shared_from_this](stdlib_and_concepts.md#SS_10_6_2_2)の使用において前提知識となっている。
...
X、Yオブジェクトへの[ハンドル](cpp_idioms.md#SS_11_12_8)を完全に失った状態であり、X、Yオブジェクトを解放する手段はない。
...
* エクセプション安全性  
    [no-fail保証](core_lang_spec.md#SS_9_13_7_1)をする(noexceptと宣言し、エクセプションをthrowしない)
...
[注]
`templat<typename T> f(T&&)`の`T&&`は[forwardingリファレンス](core_lang_spec.md#SS_9_8_3)である。  
...
### プライオリティインバージョン <a id="SS_11_12_2"></a>
...
### スレッドセーフ <a id="SS_11_12_3"></a>
スレッドセーフとは「複数のスレッドから同時にアクセスされても、
排他制御などの機構([std::mutex](stdlib_and_concepts.md#SS_10_4_2))により共有データの整合性が保たれ、正しく動作する性質」である。

### リエントラント <a id="SS_11_12_4"></a>
リエントラントとは「実行中に同じ関数が再度呼び出されても、グローバル変数や静的変数に依存せず、
ローカル変数のみで動作するため正しく動作する性質」である。

一般に、リエントラントな関数は[スレッドセーフ](cpp_idioms.md#SS_11_12_3)であるが、逆は成り立たない。


### クリティカルセクション <a id="SS_11_12_5"></a>
複数のスレッドから同時にアクセスされると競合状態を引き起こす可能性があるコード領域をクリティカルセクションと呼ぶ。
典型的には、共有変数や共有データ構造を読み書きするコード部分がこれに該当する。
クリティカルセクションは、[std::mutex](stdlib_and_concepts.md#SS_10_4_2)等の排他制御機構によって保護し、
一度に一つのスレッドのみが実行できるようにする必要がある。

### スピンロック <a id="SS_11_12_6"></a>
スピンロックとは、
...
C++11では、スピンロックは[std::atomic](stdlib_and_concepts.md#SS_10_4_3)を使用して以下のように定義できる。
...
以下の単体テスト(「[std::mutex](stdlib_and_concepts.md#SS_10_4_2)」の単体テストを参照)
に示したように[std::scoped_lock](stdlib_and_concepts.md#SS_10_5_3)のテンプレートパラメータとして使用できる。
...
### ハンドル <a id="SS_11_12_8"></a>
CやC++の文脈でのハンドルとは、ポインタかリファレンスを指す。

### フリースタンディング環境 <a id="SS_11_12_9"></a>
...
- メモリ保護違反例外:
    [メモリ保護機構](cpp_idioms.md#SS_11_12_10)の設定に反した命令を実行した場合に発生する。例えば、リードオンリー領域への書き込み、
